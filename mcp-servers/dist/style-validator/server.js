...
// Add style check tool
mcpServer.tool('style_check', {
...
// Add file style check tool
mcpServer.tool('check_file_style', {
...
async function handleRequest(req, res) {
    ...
    if (pathname === '/mcp' && req.method === 'POST') {
        ...
        try {
            ...
            if (body.name === 'style_check') {
            ...
            else if (body.name === 'check_file_style') {
                ...
                if (!filePath) {
                    ...
                    const result = await (0, style_validator_1.validateStyles)(filePath, {
    ...
    if (pathname === '/validate' && req.method === 'POST') {
        ...
        try {
            ...
            (0, utils_1.logger)('error', 'Error validating styles', { error: error.message, projectPath });
            (0, utils_1.sendJsonResponse)(res, {
                success: false,
    ...
    if (pathname === '/check-file' && req.method === 'POST') {
        ...
        if (!filePath) {
            (0, utils_1.sendJsonResponse)(res, { error: 'Missing file path' }, 400);
            return;
        }
        try {
            const result = await (0, style_validator_1.validateStyles)(filePath, { ...(body.options || {}), singleFile: true });
            (0, utils_1.sendJsonResponse)(res, result);
        }
        catch (error) {
            (0, utils_1.logger)('error', 'Error checking file style', {
                error: error.message,
                filePath
            });
            (0, utils_1.sendJsonResponse)(res, {
                success: false,
                errors: [`Error checking style: ${error.message}`],
                warnings: [],
                fixable: [],
