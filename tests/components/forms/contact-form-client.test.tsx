...
describe('ContactFormClient', () => {
  ...
  test('validates required fields and shows error messages', async () => {
    ...
    await waitFor(() => {
      const errorMessages = ['Please check the form for errors', 'Name is required', 'Email is required', 'Message must be at least 10 characters'];
      
      errorMessages.forEach(message => {
        // Convert NodeList to Array using Array.from instead of spread operator
        const errorElement = Array.from(container.querySelectorAll('p'))
          .find(p => p.textContent?.includes(message));
