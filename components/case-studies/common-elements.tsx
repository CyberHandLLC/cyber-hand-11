...
export const SectionHeader = ({ title, theme }: SectionHeaderProps) => (
  <div className="flex items-center mb-6">
    ...
    <h2 className={`text-2xl ${CASE_STUDY_STYLES.headings.h2} ml-4 ${getThemeStyle('text-primary', theme)}`}>
      {title}
    </h2>
  </div>
);
...
export const ApproachStep = ({ step, index, theme }: ApproachStepProps) => (
  <div className="relative">
    <div className={`absolute -left-6 w-10 h-10 rounded-full bg-black border ${CASE_STUDY_STYLES.borderAccent}/50 flex items-center justify-center`}>
...
export const ResultItem = ({ result, theme }: ResultItemProps) => (
  <div className={`p-6 ${CASE_STUDY_STYLES.card} ${CASE_STUDY_STYLES.cardHover}`}>
    <div className="flex items-start">
      <div className={`flex-shrink-0 w-8 h-8 rounded-full ${CASE_STUDY_STYLES.accentLight} flex items-center justify-center mr-4`}>
...
export const SidebarCard = ({ title, description, buttonText, buttonIcon, onClick, theme }: SidebarCardProps) => (
  <div className={`p-6 ${CASE_STUDY_STYLES.card} mb-8`}>
    <h3 className={`text-xl ${CASE_STUDY_STYLES.headings.h3} mb-4 text-white`}>
      {title}
    </h3>
