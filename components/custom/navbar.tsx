...
export function Navbar() {
  ...
  const AuthButtons = () => (
    <div className="flex items-center">
      <Link
        href="/login"
        className="px-5 py-2 rounded-full text-sm font-medium text-gray-300 hover:text-white transition-colors mr-2"
      >
        Log In
      </Link>
      <Link
        href="/signup"
        className="px-5 py-2 rounded-full text-sm font-medium bg-white hover:bg-gray-100 text-gray-900 transition-colors"
      >
        Sign up
      </Link>
    </div>
      ...
      <nav className="flex items-center rounded-full px-4 py-2 backdrop-blur-md bg-black/30 border border-gray-700/30 shadow-xl">
        ...
        <div className="hidden md:flex items-center space-x-8 mr-6">
          {navigationItems.map((item) => (
            <NavLink key={item.path} href={item.path} active={isActive(item.path)}>
              {item.label}
            </NavLink>
          ))}
        </div>
