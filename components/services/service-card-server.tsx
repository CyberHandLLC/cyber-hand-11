...
export function ServiceCardServer({ service, _index }: ServiceCardServerProps) {
  const { id, title, description, price, features, icon, popular } = service;
  ...
  const badgeStyle =
    "absolute -top-3 right-1/2 transform translate-x-1/2 bg-cyan-500/20 text-cyan-300 px-3 py-1 rounded-full text-xs font-semibold whitespace-nowrap border border-cyan-500/30";
  ...
  return (
    <div className={cardStyle}>
      {/* Popular badge */}
      {popular && <div className={badgeStyle}>Popular Choice</div>}
      ...
      <ul className="mb-6 space-y-2 flex-grow">
        {features.map((feature, i) => (
          <li key={i} className="flex items-start">
            <CheckIcon className={checkIconStyle} size="sm" />
            <span className={featureTextStyle}>{feature}</span>
          </li>
        ))}
      </ul>
